...
Tutorial is demonstrates the typical process of fetching account blog posts. It is quite useful if you want to embed your blog posts on your website these tutorial will help you achieve that goal as well. This tutorial will explain and show you how to access the **dPay** blockchain using the [ddpays](https://github.com/dpays/ddpays) library to build a basic blog list of posts filtered by a _tag_

## Steps

1.  [**Configure connection**](#Configure-connection) Configuration of ddpays to use proper connection and network
...
In order to connect to the live dPay network, all we have to do is provide connection url to a server that runs on the network. `ddpays` by default set up to use live network but it has flexibility to adjust connection to any other testnet or custom networks, more on that in future tutorials.

In first couple lines we require package and define connection server:

```
const { Client } = require('ddpays');
...
```javascript
var query = {
    tag: 'dpayblog', // This tag is used to filter the results by a specific post tag
...
[
    {
        ...
        "beneficiaries": [],
        "url": "/dsite/@dpayblog/join-team-dsite-at-tokenfest",
        "root_title": "Join Team dPay at TokenFest!",
